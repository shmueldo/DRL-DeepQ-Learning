...
class DoubleDeepQLearning(DeepQLearning):
    def __init__(self, environment, q_learning_rate, discount_factor, decaying_rate, epsilon,
                model_type:tf.keras.Model, criterion, optimizer, net_learning_rate, model_name, alfa, ):
        super().__init__(environment, q_learning_rate, discount_factor, decaying_rate, epsilon, model_type,
                         criterion, optimizer, net_learning_rate, model_name)
        ...
        self.model_a = self.model_init(model_type=model_type, criterion=criterion, optimizer=optimizer, net_learning_rate=net_learning_rate)
        self.model_b= self.model_init(model_type=model_type, criterion=criterion, optimizer=optimizer,
    ...
    def sample_action(self, current_state):
        ...
        if prob < self.epsilon:
            ...
            q_table = self.model_a.predict(self.model_input_reshape(current_state), verbose=0).flatten() + self.model_b.predict(self.model_input_reshape(current_state), verbose=0).flatten()
            max_q = np.where(np.max(q_table) == q_table)[0]
            return np.random.choice(max_q)

    def get_target(self, next_q_val, current_q_value, reward, done):
        if not done:
            target = current_q_value + self.alfa * (reward + self.discount_factor * np.max(next_q_val) - current_q_value)
        else:
            target = reward
        return target

    def generate_training_database(self, batch_size, model_name):
        """generates datasets for the main model training \
           where the input data is the current states and the "labels" \
           are updated modified q values, based on Bellman equation
        ...
        next_states = np.array([batch_data[4] for batch_data in mini_batch])
        if model_name == 'a':
            current_q_val = self.model_a.predict(current_states, verbose=0)
            next_q_val = self.model_b.predict(next_states, verbose=0)
        elif model_name == 'b':
            current_q_val = self.model_b.predict(current_states, verbose=0)
            next_q_val = self.model_a.predict(next_states, verbose=0)
        ...
        for i, exp_sample in enumerate(mini_batch):
            ...
            # get target based on target model
            target = self.get_target(next_q_val[i], current_q_val[i], reward, done)
    ...
    def train(self, batch_size, step, model_name):
        observations, q_values = self.generate_training_database(batch_size=batch_size, model_name=model_name)
        # train model
        if model_name == 'a':
            self.model_a.fit(x=observations, y=q_values,
                                       batch_size=batch_size,
                                       verbose=0,
                                       callbacks=[self.tensorboard])
        elif model_name == 'b':
            self.model_b.fit(x=observations, y=q_values,
                             batch_size=batch_size,
                             verbose=0,
                             callbacks=[self.tensorboard])
    ...
    def train_agent(self, num_of_episodes, weights_assign_num, training_num,
        ...
        for episode in tqdm(range(num_of_episodes)):
            ...
            while not terminated:
                ...
                #get r and s'
                next_state, reward, terminated = self.env_step(current_action)
                ...
                #Update(model_a) or Update(model_b)
                if update_counter % training_num == 0:
                    if len(self.exp_reply_deque) >= self.minimal_reply_size and len(self.exp_reply_deque) >= batch_size:
                        prob = np.random.rand()
                        if prob < 0.5:
                            self.train(batch_size=batch_size, step=overall_steps, model_name='a')
                        else:
                            self.train(batch_size=batch_size, step=overall_steps, model_name='b')
