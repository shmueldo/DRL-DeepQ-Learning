...
def main():
    ...
    # !: initialize the main net (intended for inference)
    model = agent(env.observation_space.shape, env.action_space.n)
    
    # !: initialize the target net (intended for inference) 
    # Target Model (updated every 100 steps)
    target_model = agent(env.observation_space.shape, env.action_space.n)
    
    # !: at the beginning, set the weights of the main to the target network
    target_model.set_weights(model.get_weights())

    # ! generate a deque with maximal size of 50K
    # TODO: what is deque ?
    ...
    for episode in range(train_episodes):
        ...
        while not done:
            ...
            # ! commit a step in the game
            new_observation, reward, done, info = env.step(action)

            # ! append parameters to the experience reply 
